    ...
    return (
        <>
            <div onMouseEnter={itemEntered} onMouseLeave={itemExited}
                className={`${props.color} ${props.padding} ${props.boxType} ${props.width} h-fit text-center ${props.other} hover:text-gray-200 ${isHovered ? `${props.hoveredColor} font-bold shadow-xl` : ''}`} >
                {props.children}
            </div>
        </>
    );
